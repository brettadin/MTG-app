...
- [ ] **Use Full State-Based Actions**
  ...
  - [x] **(Session 17)** Replace temporary `engine.test_mode` guard added during testing with a dedicated test harness/fixture to ensure deterministic SBA order without production-side flags. ✅ Completed: `resolve_stack_and_check_sbas` helper and `resolve_stack_fixture` in `tests/conftest.py`.
...
- [ ] **State-Based Actions Tests** - state_based_actions.py
  - [x] Investigate recurring flakiness in `test_sbas_checked_after_spell_resolves` (investigated in Session 17). Replaced reliance on `engine.test_mode` in tests with `resolve_stack_and_check_sbas` harness. ✅ Completed
